...
function Header() {
  ...
  const handleLogout = () => {
    if (window.confirm('Çıxış etmək istəyirsiniz?')) {
      dispatch(logout());
      navigate('/login');
    }
  };
      ...
      <header className={styles.header}>
          ...
          <div className={styles.right}>
            {user && (
              <>
                ...
                <Link to="/profile">
                  {user.profileImage ? (
                    <img src={`http://localhost:5555/uploads/${user.profileImage}`} alt="P" className={styles.profileImage} />
                  ) : (
        ...
        {isMobile && menuOpen && (
          <nav className={styles.dropdownMenu}>
            ...
            <button onClick={handleLogout} className={styles.logoutBtn}><FiLogOut /> Logout</button>
            <button onClick={() => dispatch(toggleTheme())} className={styles.themeToggle}>
              {theme === 'dark' ? <FiSun /> : <FiMoon />}
            </button>
          </nav>
